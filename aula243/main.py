...
class MyWidget(QWidget, Ui_myWidget):
    ...
    def hardWork1(self):
       ...
       # Isso garante que o widget vai ter a referência para o worker e thread
       worker = self._worker1
       thread = self._thread1

       # Worker é movido para a  thread. Todas as informaçãoes e métodos
    ...
    def hardWork2(self):
       ...
       # Isso garante que o widget vai ter a referência para worker e thread
       worker = self._worker2
       thread = self._thread2

       # Worker é movido para a thread. Todas as funções e métodos do objeto
       # de worker serão executados na thread criado pela QThread.
       worker.moveToThread(thread)

       # Quando uma QThread é iniciada, emite um sinal started automaticamente.
       # Nome do método "doWork" modificado para "executeMe" - por exemplo
       thread.started.connect(worker.executeMe) # type: ignore
       
       # O sinal finished é emitido pelo objeto worker quando o trabalho que ele
       # está executando é concluído. Isso aciona o método quit da qthread que
       # interrompe o loop de eventos dela.
       worker.finished.connect(thread.quit)
    ...
    def worker2Finished(self, value):
        ...
        print('2 worker finalizado', value)  
